...
export default function Shipments() {
  ...
  async function load() {
    ...
    try {
      ...
      setShipments(Array.isArray(res) ? res : res?.shipments || [])
    } catch { setShipments([]) }
    setLoading(false)
  }
  ...
  async function saveEdit(id) {
    ...
    try {
      ...
      setShipments(prev => prev.map(s => s.id === id ? { ...s, ...editData } : s))
      setEditing(null)
  ...
  const filtered = shipments.filter(s => {
    ...
    const matchSearch = !search || [s.tracking_number, s.origin_city, s.destination_city, s.carrier_name].some(v => v?.toLowerCase().includes(search.toLowerCase()))
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
              <thead>
                <tr style={{ borderBottom: `1px solid ${C.border}` }}>
                  {["ID", "Origin", "Destination", "Carrier", "Status", "ETA", "Actions"].map(h => (
              ...
              <tbody>
                {filtered.length === 0 ? (
                  <tr><td colSpan={7} style={{ padding: 32, textAlign: "center", color: C.textLow }}>No shipments found</td></tr>
                ) : filtered.map((s, i) => (
                  <>
                    <tr key={s.id} style={{ borderBottom: `1px solid ${C.border}`, background: i % 2 === 0 ? "transparent" : "rgba(0,0,0,0.06)" }}>
                      ...
                      <td style={{ padding: "11px 14px", color: C.textMid }}>{s.destination_city || "—"}</td>
                      <td style={{ padding: "11px 14px", color: C.textMid }}>{s.carrier_name || "—"}</td>
                      ...
                      <td style={{ padding: "11px 14px", color: C.textMid, whiteSpace: "nowrap" }}>{s.estimated_delivery ? new Date(s.estimated_delivery).toLocaleDateString("en-IN") : "—"}</td>
                      <td style={{ padding: "11px 14px" }}>
                        <button onClick={() => { setEditing(editing === s.id ? null : s.id); setEditData({ status: s.status, carrier_name: s.carrier_name, estimated_delivery: s.estimated_delivery }) }}
                          style={{ padding: "4px 12px", borderRadius: 7, fontSize: 12, fontWeight: 600, background: editing === s.id ? C.accentDim : "rgba(255,255,255,0.07)", border: `1px solid ${editing === s.id ? C.accent : C.border}`, color: editing === s.id ? C.accent : C.textMid }}>
                    ...
                    <AnimatePresence>
                      {editing === s.id && (
                        <motion.tr key={`edit-${s.id}`} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                          <td colSpan={7} style={{ padding: "14px 16px", background: "rgba(0,0,0,0.15)", borderBottom: `1px solid ${C.border}` }}>
                            <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "flex-end" }}>
                              ...
                              <div>
                                ...
                                <input value={editData.carrier_name || ""} onChange={e => setEditData(p => ({ ...p, carrier_name: e.target.value }))}
                              ...
                              <div>
                                ...
                                <input type="date" value={editData.estimated_delivery?.slice(0,10) || ""} onChange={e => setEditData(p => ({ ...p, estimated_delivery: e.target.value }))}
                                  style={{ padding: "7px 12px", borderRadius: 8, background: "rgba(0,0,0,0.3)", border: `1px solid ${C.border}`, color: C.textHi, fontSize: 13, outline: "none" }} />
                              </div>
                              <button onClick={() => saveEdit(s.id)} disabled={saving}
                                style={{ padding: "8px 20px", borderRadius: 8, background: C.grad, color: "#fff", fontWeight: 700, fontSize: 13, border: "none" }}>
                                {saving ? "Saving..." : "Save"}
                              </button>
                            </div>
                          </td>
