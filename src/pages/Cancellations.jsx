...
export default function Cancellations() {
  ...
  async function load() {
    setLoading(true)
    try {
      const res = await api.getCancellations()
      setItems(Array.isArray(res) ? res : res?.cancellations || [])
    } catch { setItems([]) }
    setLoading(false)
  }
      ...
      <div style={{ display: "flex", gap: 4, marginBottom: 20 }}>
        {["pending", "approved", "rejected", "all"].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            style={{ padding: "6px 16px", borderRadius: 8, fontSize: 12, fontWeight: 600, border: `1px solid ${filter === f ? C.accent : C.border}`, background: filter === f ? C.accentDim : "transparent", color: filter === f ? C.accent : C.textMid }}>
              ...
              <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
                <div style={{ flex: 1 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                    ...
                    <span style={{ padding: "2px 9px", borderRadius: 99, fontSize: 11, fontWeight: 700, background: item.status === "pending" ? `${C.warning}18` : item.status === "approved" ? `${C.success}18` : `${C.error}18`, color: item.status === "pending" ? C.warning : item.status === "approved" ? C.success : C.error }}>
                      {item.status?.toUpperCase()}
                    </span>
                  </div>
                  <div style={{ color: C.textMid, fontSize: 13, marginBottom: 4 }}><span style={{ color: C.textLow }}>Customer:</span> {item.profiles?.full_name || item.profiles?.email || "—"}</div>
                  <div style={{ color: C.textMid, fontSize: 13, marginBottom: 4 }}><span style={{ color: C.textLow }}>Reason:</span> {item.reason || "No reason provided"}</div>
                  <div style={{ color: C.textLow, fontSize: 12 }}>Requested: {item.created_at ? new Date(item.created_at).toLocaleString("en-IN") : "—"}</div>
                </div>
                {item.status === "pending" && (
                  <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
                    <button onClick={() => approve(item.id)} disabled={acting === item.id}
                      style={{ padding: "7px 18px", borderRadius: 8, background: `${C.success}18`, border: `1px solid ${C.success}40`, color: C.success, fontSize: 13, fontWeight: 700 }}>
                      {acting === item.id ? "..." : "Approve"}
                    </button>
                    <button onClick={() => { setRejectId(rejectId === item.id ? null : item.id); setRejectNote("") }}
                      style={{ padding: "7px 18px", borderRadius: 8, background: `${C.error}18`, border: `1px solid ${C.error}40`, color: C.error, fontSize: 13, fontWeight: 700 }}>
                      Reject
                    </button>
                  </div>
                )}
                  ...
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <input value={rejectNote} onChange={e => setRejectNote(e.target.value)} placeholder="Enter reason..."
                        style={{ width: "100%", padding: "8px 12px", borderRadius: 8, background: "rgba(0,0,0,0.2)", border: `1px solid ${C.border}`, color: C.textHi, fontSize: 13, outline: "none" }} />
                    </div>
                    <button onClick={() => reject(item.id)} disabled={acting === item.id}
                      style={{ padding: "8px 18px", borderRadius: 8, background: C.error, color: "#fff", fontWeight: 700, fontSize: 13, border: "none" }}>
