      ...
      <div style={{ display: "flex", gap: 4, marginBottom: 20 }}>
        {["pending", "approved", "rejected", "all"].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            style={{ padding: "6px 16px", borderRadius: 8, fontSize: 12, fontWeight: 600, border: `1px solid ${filter === f ? C.accent : C.border}`, background: filter === f ? C.accentDim : "transparent", color: filter === f ? C.accent : C.textMid, transition: "all 0.15s" }}>
              ...
              <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
                <div style={{ flex: 1 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8, flexWrap: "wrap" }}>
                    ...
                    <span style={{ padding: "2px 9px", borderRadius: 99, fontSize: 11, fontWeight: 700, background: item.status === "pending" ? `${C.warning}18` : item.status === "approved" ? `${C.success}18` : `${C.error}18`, color: item.status === "pending" ? C.warning : item.status === "approved" ? C.success : C.error, border: `1px solid ${item.status === "pending" ? C.warning : item.status === "approved" ? C.success : C.error}35` }}>
                      {item.status?.toUpperCase()}
                    </span>
                  </div>
                  <div style={{ color: C.textMid, fontSize: 13, marginBottom: 4 }}>
                    <span style={{ color: C.textLow }}>Customer:</span> {item.customer_name || item.user_email || "—"}
                  </div>
                  <div style={{ color: C.textMid, fontSize: 13, marginBottom: 4 }}>
