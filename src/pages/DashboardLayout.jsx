...
// ── Command parser — no AI needed for direct actions ──────
function parseCommand(input, context) {
  const txt = input.toLowerCase().trim()

  // Resolve shipment by tracking number
  const findShipment = (q) => context?.shipments?.find(s =>
    s.tracking_number?.toLowerCase().includes(q) ||
    s.id?.toLowerCase().includes(q)
  )
  const findCancellation = (q) => context?.cancellations?.find(c =>
    c.tracking_number?.toLowerCase().includes(q) ||
    c.id?.toLowerCase().includes(q)
  )

  // Extract shipment ID from text
  ...
  if (shipId) {
    ...
    if (txt.includes("in_transit") || txt.includes("in transit") || txt.includes("dispatch") || txt.includes("ship")) {
      return { type: "status_update", shipment: s, status: "in_transit", message: `Marking ${shipId} as In Transit` }
    }
    if (txt.includes("deliver") || txt.includes("complete") || txt.includes("done")) {
      return { type: "status_update", shipment: s, status: "delivered", message: `Marking ${shipId} as Delivered` }
    }
    if (txt.includes("delay") || txt.includes("flag")) {
      return { type: "status_update", shipment: s, status: "delayed", message: `Flagging ${shipId} as Delayed` }
    }
    if (txt.includes("cancel")) {
      return { type: "status_update", shipment: s, status: "cancelled", message: `Cancelling ${shipId}` }
    }
    if (txt.includes("pending") || txt.includes("reset")) {
      return { type: "status_update", shipment: s, status: "pending", message: `Resetting ${shipId} to Pending` }
    }
    if (txt.includes("approve") || txt.includes("confirm cancell")) {
      const c = findCancellation(shipId.toLowerCase())
      if (c) return { type: "approve_cancel", cancellation: c, message: `Approving cancellation for ${shipId}` }
    }
    if (txt.includes("reject")) {
      const c = findCancellation(shipId.toLowerCase())
      if (c) return { type: "reject_cancel", cancellation: c, message: `Rejecting cancellation for ${shipId}` }
    }
  }

  // APPROVE ALL PENDING CANCELLATIONS
  if ((txt.includes("approve") || txt.includes("confirm")) && txt.includes("all") && (txt.includes("cancel") || txt.includes("pending"))) {
  ...
  // SUMMARY / STATUS — send to AI
  if (txt.includes("summary") || txt.includes("overview") || txt.includes("report") || txt.includes("status") || txt.includes("how many") || txt.includes("which") || txt.includes("list") || txt.includes("show")) {
...
export default function DashboardLayout() {
  ...
  async function loadContext() {
    const [ships, cans] = await Promise.all([
    ...
    setContext({
      shipments: s.map(x => ({ uuid: x.id, id: x.tracking_number || x.id, tracking_number: x.tracking_number, status: x.status, origin_city: x.origin_city, dest_city: x.dest_city, customer: x.profiles?.full_name || x.profiles?.email || "Unknown", carrier: x.carrier, eta: x.eta })),
      cancellations: c.map(x => ({ id: x.id, tracking_number: x.tracking_number, status: x.status, reason: x.reason, customer: x.profiles?.full_name || x.profiles?.email || "Unknown", shipment_id: x.shipment_id }))
    })
  }

  useEffect(() => { bottomRef.current?.scrollIntoView({ behavior: "smooth" }) }, [messages, open])

  function addMsg(role, content, color) {
    setMessages(prev => [...prev, { role, content, color }])
  }
  ...
  async function send() {
    ...
    try {
      ...
      if (cmd.type === "status_update") {
        ...
        setMessages(prev => {
          const msgs = [...prev]
          msgs[msgs.length - 1] = { role: "assistant", content: `✓ ${cmd.message.replace("⏳ ", "").replace("...", "")} — Done!`, color: C.success }
          return msgs
        })
        await loadContext()
        ...
        // AI fallback for questions/summaries
  ...
  return (
    <ChatContext.Provider value={{ messages, setMessages, input, setInput, loading, send, context, open, setOpen, bottomRef, INIT }}>
            ...
            <div style={{ padding: "12px 16px", background: "#332B7A", borderBottom: "1px solid rgba(255,255,255,0.1)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div>
                ...
                <div style={{ fontSize: 10, color: context ? C.success : C.warning, marginTop: 1 }}>
                  {context ? `${context.shipments.length} shipments · ${context.cancellations.length} cancellations loaded` : "Loading data..."}
                </div>
              </div>
              ...
              {messages.map((m, i) => (
                <div key={i} style={{ display: "flex", justifyContent: m.role === "user" ? "flex-end" : "flex-start" }}>
                  <div style={{
                    ...
                    background: m.role === "user" ? C.grad : "rgba(255,255,255,0.07)",
                    border: m.role === "user" ? "none" : `1px solid ${m.color ? m.color + "40" : "rgba(255,255,255,0.1)"}`,
                    color: m.color || C.textHi,
